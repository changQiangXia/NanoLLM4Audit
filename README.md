...
- 适用场景：课程展示、论文原型说明、实验室面试讲解、快速复现实验
...
`data/demo_logs.csv` 由固定随机种子生成，包含 334 条演示日志，其中既有稳定业务行为，也有多段攻击链片段。固定种子带来一致的图表与指标输出。演示数据中的风险特征保持清晰，适合首轮阅读和课堂展示。
...
在当前项目中，`R = 7`，`N = 334`，因此运行开销较低，适合本地快速演示。
